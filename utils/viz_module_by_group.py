...
def visualize_instance(trips_list):
    ...
    route_id_list = list(set([trip.route_id for trip in trips_list]))
    routes_group = [i for i in range(len(route_id_list))]
    for route_index, route_id in enumerate(route_id_list):
        relevant_trip_list = [
            trip for trip in trips_list if trip.route_id == route_id]
        routes_group[route_index] = folium.FeatureGroup(
            name=route_id, show=True).add_to(some_map)
        for trip in relevant_trip_list:
            for index, stop in enumerate(trip.stops_list):
                if stop == trip.stops_list[0]:
                    next_stop = trip.stops_list[index+1]
                    routes_group[route_index].add_child(folium.CircleMarker(
                    ...
                    routes_group[route_index].add_child(folium.PolyLine(
